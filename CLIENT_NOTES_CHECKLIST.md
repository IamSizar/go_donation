...
| B3 | Place detail shows a **raw JSON array** `["commercial","government","education","health"]` instead of readable sector names | Dashboard → دليل المدينة → عرض | 🛠️ **fixed** (earlier commit; verified here) — list values now read as Arabic words separated by an Arabic comma. Fixed generically, so the same fault cannot appear on volunteer skill tags, marketplace labels or privacy fields either |
...
| B6 | General: "انكليزي لم يتم الترجمه" — untranslated English remains in places. PDF restates it as a global requirement: "إلزام النظام بتعريب وترجمة كافة الكلمات والعبارات التي ما زالت تظهر باللغة الإنجليزية في جميع قوائم لوحة التحكم عند اختيار اللغة العربية أو اللغة الكردية" — client wants it fixed once as a Global Function. `[D p2, p3]` | App + Dashboard | 🛠️ **fixed** — the last English left in the Arabic dashboard was the grey hint text inside empty boxes, which bypassed the translation mechanism the labels already used. All 11 are converted. The hints that stay in Latin (`IQD`, `0`, `YYYY-MM-DD`, a JSON sample, a URL sample) are format examples with no word in them to translate |
...
| B9 | Same page shows **raw ISO timestamps** `2026-06-14T11:13:32.50385Z` instead of a localized Arabic date/time. `[D p1]` | Dashboard → المستخدمون → عرض | 🛠️ **fixed** (earlier commit; verified here) — dates now use the same formatter the rest of the dashboard uses. It recognises the date by its shape, not by the column name, so a new date column is formatted without anyone adding it to a list |
...
| B21 | **Choosing a language must translate ALL app text** — "عند اختيار لغة معينة يجب أن يكون جميع الكلام الموجود داخل التطبيق باللغة التي تم اختيارها". Test by switching each of the four languages and sweeping every screen. `[A p34]` | App | 🛠️ **Arabic done; Kurdish still needs a translator** — 19 English sentences with no Arabic behind them are translated (الكفالات, مساهماتي, empty chat, the Services list, the volunteer mission cards), and the marketplace order card noted below is fixed. Rather than promise "that's all of them", a test now checks the whole Arabic map every build: no key without Arabic, no Arabic that is really English, no Latin letters in an Arabic label. Kurdish deliberately still falls back to English — 236 words are itemised in TRANSLATION_REQUEST.md for a native speaker |
...
| E17 | **Profile edits must not save until an employee approves** — request goes to the dashboard; "لا تُعتمد التغييرات إلا بعد مراجعتها والموافقة عليها من قبل موظف التطبيق"; employee can accept or reject with a stated reason. `[A p26, p34]` | App → تعديل الملف الشخصي + Dashboard | 🛠️ **half fixed, half needs your decision** — the app no longer claims a change was saved when it was actually sent for review; it now says which parts are waiting and which are already saved, and it stops showing you a photo nobody has approved yet. What still needs YOU: only the **name** and **photo** go to review. An **address** change saves instantly, and **deleting** a photo takes effect with no review at all. Tell us whether those two should wait for an employee as well |
...
| # | Item | Where | Status |
|---|---|---|---|
| J1 | **Guest-login button was never added** — "لم يتم إضافة زر الدخول كزائر". Guest entry asks only for الاسم، اسم المستخدم (Username) فقط، رمز سري, and "حفظ بيانات التسجيل في النظام والتقارير". `[A p1]` | App → home / login | 🛠️ **fixed** — commit `7a449ee`. The sheet already had اسم المستخدم + رمز سري; the missing الاسم box is now there and posts `full_name` on the REGISTER call (`GuestLogin` parses the name and never reads it, so the log-in-instead path deliberately sends none). Optional, 200-rune limit matching the column, `pf_full_name` label — a key that already exists in all four locales. Backend `792ded3` is what made it storable: `InsertGuest` wrote no `user_profiles` row at all before it |
...
| K7 | **Settings page**: language selection across **العربية، الإنجليزية، الكردية البادينية، الكردي الصوراني**; notifications on/off plus per-type control of what the user receives. `[A p24]` | App → الإعدادات | 🛠️ **fixed** — commit `7bc57b3`. The four languages were already correct. Per-type control is now a فئات التنبيهات screen under the الإشعارات row: one switch per CATEGORY (عاجل، دفع، حملة، النظام، تذكير، عادي), enforced server-side inside `notify.Send` (`159a3b2` + migration 108) so switching one off stops the alert being SENT, not merely hidden. Category rather than raw type because there are 81 types and the server derives a category from every one. The master on/off switch is on the same screen, since it still overrides every category |
...
| K12 | **من نحن details**: نبذة مختصرة عن التطبيق، عن المنظمة، عن أهداف المنظمة، plus the ability to add more fields later without code. `[A p28]` | App → من نحن | 🛠️ done end to end — server `1696562` + migration 111 (named, ordered, extendable sub-sections) and dashboard editor; app `472f937` draws each as its own titled block, and a page with no text says so instead of going blank. **Owner still has to write the three sections' text** |
| K13 | **تواصل معنا details**: لوكو المنظمة، رقم هاتف المنظمة، رقم الواتساب، الإيميل، مواقع التواصل، عنوان وموقع المنظمة, extendable fields. `[A p28]` | App → اتصل بنا | 🛠️ done end to end — server `cea459b` + migration 112 (all six fields, validated both sides) and dashboard editor; app `34aae78` draws whichever exist and **nothing at all for the ones that do not**, with phone/WhatsApp/email actionable and no dead links. **Owner still has to supply the real values** |
| K14 | **خطوبتي/الزواج feature set**: account creation per the L15–L19 spec, login, guest registration, a profile with edit/activate/delete-account, **filtered search**, **save-a-person during search**, and a contact request routed either as an interview, through a mediating employee, or as a visit request — plus subscription and the documented payment methods. `[A p28]` | App → خطوبتي | ✅ both halves — owner edit/pause/resume/recoverable-delete endpoints + migration 110, and the app buttons that call them (edit screen, status-gated pause/resume, confirmed recoverable removal, failures localized from the server's machine `code`) |
| K15 | **Products list structure**: 10 main categories (الإلكترونيات والأجهزة المنزلية، الأزياء والملابس، الصحة والجمال، المنزل والحديقة، المواد الغذائية والمشروبات، الرياضة واللياقة البدنية، ألعاب الأطفال والمواليد، مستلزمات السيارات، الكتب والمكتبة والقرطاسية، العدد والأدوات الصناعية); functional labels الأكثر مبيعاً، وصل حديثاً، العروض والخصومات، التصفية، الفئات، العلامات التجارية; per-product fields اسم المنتج، الوصف، رمز التخزين التعريفي، السعر، الحالة/التوفر، المواصفات التقنية. `[A p29]` | App → منتجاتنا | ✅ both halves — server sorts/filters (migration 109) and the app's six label chips, every one of them a server query rather than a re-sort of the loaded page; 2 owner decisions still open — see VERIFICATION_REPORT.md K15 |
...
| K19 | **Supervised donor↔beneficiary chat**: opened only on the donor's request and with the assigned employee's approval, the employee acting as mediator and monitor, with **full personal-data blocking "منعاً للابتزاز"**. `[A p33]` | App | 🛠️ **server half done — the personal-data blocking, both directions.** The thread, the staff relay and the staff claim already existed. What did not: (1) **the write path** now refuses a message carrying a phone number or an email address (422 + `code: contact_details_blocked`), handling Arabic-Indic and extended Arabic-Indic digits, separators and zero-width/bidi evasion, while donation amounts, `CAM-000042`, `CSE-2026-001` and dates pass untouched; (2) **the read path** was handing each party the other's real `phone` in `GET /api/chats` — withheld now on peer threads, which made (1) worth having. Staff senders and support threads are exempt. Migration **116** records refused attempts with the number redacted. **App must**: switch on the 422 `code`+`kind` and render the three localized sentences in TRANSLATION_REQUEST.md — today it would show the English fallback. **Still open (owner decision, not built):** the thread is approved by the *other party*, not by an employee — `AcceptThread` requires the counterpart — so "with the assigned employee's approval" is unmet; and the counterpart's real *name* is still served. See VERIFICATION_REPORT.md K19 |
| K20 | **General chat platform** reaching all user types (مستفيدين، متبرعين، وزوار), plus a **confidential employee-only space** for exchanging notes and receiving smart task alerts from management. `[A p33]` | App | 🛠️ **employee space was already done; the زوار gap is now closed server-side.** Verified before building: `internal/staffchat` (migration 059) already gives every dashboard tier staff↔staff threads/messages/reads + a directory, and `notify.TaskAssignedMsg` already fans management's task assignments out to each assignee — that is both halves of the "confidential employee-only space", and nothing was needed. The real gap was **زوار**: every chat write route carried `RequireNotGuest()` including `/chats/support`, while Section 27 ships the `support` guest screen **enabled by default** — the app promised visitors support and the server refused it. **Deliberately NOT built: a general chat for guests.** A guest account is a username + 6-character password with no phone and no OTP, so an open guest channel is an unmoderated spam/harassment surface aimed at beneficiaries. A guest may now reach **staff only**, and the moderation is the existing flow: the thread opens `pending` and a staff member must accept before the guest can send anything. **App must**: call `POST /api/chats/support` from the guest support screen, render the `pending` wait state, and localize the new 403 sentence (TRANSLATION_REQUEST.md). **Owner decisions:** should the employee chat also live in the phone app (today it is dashboard-only), and should a guest's support request be flagged to staff differently? See VERIFICATION_REPORT.md K20 |
| K21 | **Smart search engine**: query by identity code to review the donation/support history — "استعلام بالكود التعريفي لاستعراض سجل (التبرعات وحالة الدعم)" — plus fast activity search. `[A p33]` | App | 🛠️ done end to end — server `5eee718` + migration 113 (`Account.identity_code`, `GET /api/history?code=`); app `a52bbb6` shows the account its own code on the profile card (tap to copy) and sends `?code=` from the history screen, with the server's deliberately vague 404 rendered as a sentence. **Owner decision open:** whether `perm("users","view")` is the right gate for reading someone else's record |
...
| L19 | **خطوبتي — إعدادات الخصوصية**: the user picks which data others may see — الاسم، العمر، الشهادة الدراسية، الوظيفة، المحافظة، الصورة الشخصية، أو أي بيانات أخرى يحددها المستخدم. `[A p13]` | App → خطوبتي profile | 🛠️ **fixed** — commit `07b5150`. خطوبتي → ملفي → خصوصية الحقول, on each profile card: a switch per detail, with the masking done server-side in `marriage.Store.List` (`0206ca0` + migration 107) so a hidden field really disappears from other people's cards. **Scope, stated rather than glossed:** the catalogue covers the ten fields the browse feed actually shows — الصورة، العمر، الجنس، المدينة، الحالة الاجتماعية، الدين، حالة العمل، الوزن، الطول، النبذة. الاسم is not among them because an engagement profile is identified by its `profile_code` and has no name to hide, and الشهادة الدراسية / الوظيفة / المحافظة are not in the browse query at all, so there is nothing there to hide yet. A new switch is an INSERT into `marriage_privacy_field_options` — no app release — but exposing those three fields would need the browse query to serve them first |
...
| # | Item | Where | Status |
|---|---|---|---|
| M1 | **Transaction Code on every payment operation**, with **independent code ranges per section** — التبرعات العامة، منجزاتنا، خطوبتي او الزواج، المشاريع، and any future section — "لتسهيل تتبع العمليات المالية والتمييز بينها في التقارير ولوحة التحكم". `[A p14–15]` | App + Backend + Dashboard | 🛠️ **fixed for المشاريع; منجزاتنا has nothing to pay for** — donations to a named project now carry their own `PRJ-` reference instead of being filed with the general fund, and the arrival SMS goes to the projects contact rather than the general one. التبرعات العامة (`GEN-`), الحملات (`CAM-`), خطوبتي والزواج (`MRG-`) and منتجاتنا (`PRD-`) were already correct. **منجزاتنا** is a read-only page in the app with no payment on it, so no code can be issued there until you add one — tell us if you intend to |
...
| M9 | **Reminders**: automatic alerts to the **donor** for his next donation date and to the **beneficiary** for his aid collection date, via in-app notification, SMS, and an in-app voice alert when available. `[A p19]` | App + Backend | 🛠️ **the previous report was wrong about SMS; voice is now seamed and a dormant spam bug is fixed.** Both sides ARE reminded automatically and **in-app genuinely works**. But **SMS has never sent anything in any environment** — `main.go`'s sender returns `nil` (success) when OTPIQ is unconfigured, and `OTPIQ_API_KEY`/`OTPIQ_SENDER_ID` are blank in `.env.example`; every caller was told the text went out. Scope is also narrower than the row reads: both reminders are about **sponsorship occurrences only** — there is no generic "next donation date" (`next_donation`, `collection_date`: zero matches) and `donations.is_recurring` is never read. **Defect found and fixed:** an occurrence whose sponsorship has no beneficiary case never had its recipient side stamped, so it was re-selected every sweep and would have re-texted the donor **every six hours forever** the moment SMS credentials landed. **Voice:** confirmed absent, now a tested seam (`internal/notify/voice.go`) driven by the sweep — **no provider, no dependency**, one line in `main.go` to switch on, and an unconfigured channel returns an error rather than faking success. **Owner decisions:** which voice provider; whether "next donation date" should cover recurring donations too; and whether to delete the dormant duplicate `internal/scheduler` before `RUN_SCHEDULER=1` double-notifies every donor. See VERIFICATION_REPORT.md M9 |
