// MediaCategoriesPage — admin CMS for the "Our Work" categories that tag News
// & Activities posts (#22). Add / edit (4 languages) / toggle active / reorder
// / delete. GET/POST/PATCH/reorder/DELETE /api/admin/media-categories.
...
export default function MediaCategoriesPage() {
  ...
  const load = () => {
    setLoading(true)
    api
      .get<{ items: Category[] }>('/api/admin/media-categories')
      .then((res) => {
  ...
  const save = async (c: Category) => {
    if (!c.name_en.trim()) {
      toast.error(t('mediaCategories.need_en'))
      return
    }
    setSavingId(c.id)
    try {
      await api.patch(`/api/admin/media-categories/${c.id}`, {
        name_en: c.name_en,
        name_ar: c.name_ar,
        name_ckb: c.name_ckb,
        name_kmr: c.name_kmr,
      ...
      toast.success(t('mediaCategories.saved'))
      load()
    } catch (e) {
  ...
  const remove = async (id: number) => {
    if (!(await askToConfirm({ message: t('mediaCategories.confirm_delete'), destructive: true }))) return
    try {
      await api.delete(`/api/admin/media-categories/${id}`)
      toast.success(t('mediaCategories.deleted'))
      load()
    } catch (e) {
      toast.error(describeError(e))
    }
  ...
  const addNew = async () => {
    ...
    setAdding(true)
    try {
      await api.post('/api/admin/media-categories', draft)
      toast.success(t('mediaCategories.added'))
  ...
  const move = async (index: number, dir: -1 | 1) => {
    ...
    setItems(reordered)
    try {
      await api.post('/api/admin/media-categories/reorder', {
  ...
  return (
    <div className="stack">
      ...
      <div className="card">
        <h3>{t('mediaCategories.add_new')}</h3>
        ...
        <button className="btn primary" onClick={addNew} disabled={adding}>
          {adding ? t('common.saving') : t('mediaCategories.add_new')}
        </button>
      </div>
        ...
        items.map((c, i) => (
          ...
          >
            ...
            >
              ...
              />
              <span className="muted">{t('mediaCategories.active')}</span>
            </label>
