// ProjectCategoriesPage — admin CMS for the project categories a beneficiary
...
export default function ProjectCategoriesPage() {
  ...
  const load = () => {
    setLoading(true)
    api
      .get<{ items: Category[] }>('/api/admin/project-categories')
      .then((res) => {
  ...
  const save = async (c: Category) => {
    if (!c.name_en.trim()) {
      toast.error(t('projectCategories.need_en'))
      return
    }
    setSavingId(c.id)
    try {
      await api.patch(`/api/admin/project-categories/${c.id}`, {
        name_en: c.name_en,
        name_ar: c.name_ar,
        name_ckb: c.name_ckb,
        name_kmr: c.name_kmr,
      ...
      toast.success(t('projectCategories.saved'))
      load()
    } catch (e) {
  ...
  const addNew = async () => {
    ...
    setAdding(true)
    try {
      await api.post('/api/admin/project-categories', draft)
      toast.success(t('projectCategories.added'))
  ...
  const move = async (index: number, dir: -1 | 1) => {
    ...
    setItems(reordered)
    try {
      await api.post('/api/admin/project-categories/reorder', {
  ...
  return (
    <div className="stack">
      <div className="page-head">
        <div>
          <h1>{t('projectCategories.title')}</h1>
          <p className="muted">{t('projectCategories.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="card">
        <h3>{t('projectCategories.add_new')}</h3>
        ...
        <button className="btn primary" onClick={addNew} disabled={adding}>
          {adding ? t('common.saving') : t('projectCategories.add_new')}
        </button>
      </div>
        ...
        items.map((c, i) => (
          <div className="card" key={c.id}>
            ...
            >
              ...
              />
              <span className="muted">{t('projectCategories.active')}</span>
            </label>
