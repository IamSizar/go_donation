// TermsPage — edit the Terms & Conditions (and other content pages) shown in
// the mobile app. Super-Admin only; the backend enforces RequireSuperAdmin.
// Loads GET /api/content/terms and saves via PUT /api/admin/content/terms.
...
// locale suffix → (label i18n key, RTL?)
...
export default function TermsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    api
      .get<{ content: Content }>('/api/content/terms')
      .then((res) => {
        if (cancelled) return
  ...
  const save = async () => {
    setSaving(true)
    try {
      await api.put('/api/admin/content/terms', form)
