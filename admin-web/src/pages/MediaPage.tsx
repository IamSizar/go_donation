...
const MEDIA_CSV_COLUMNS: CsvColumn<MediaPost>[] = [
  ...
  { header: 'title_sorani', get: (m) => m.title_sorani },
  { header: 'title_badini', get: (m) => m.title_badini },
  { header: 'post_type', get: (m) => m.post_type },
...
export default function MediaPage() {
  ...
  const requestKey = `${status}|${postType}|${q}|${refreshTick}`
  const loading = loadedKey !== requestKey

  useEffect(() => {
    let cancelled = false
    api
      .get<Resp>('/api/media', { params: { status, type: postType, q: q || undefined, limit: 100 } })
      .then((res) => {
  ...
  }, [status, postType, q, refreshTick, requestKey])
  ...
  const handleSave = useCallback(
    async (id: number, patch: Record<string, unknown>) => {
      ...
      toast.success(t('toast.saved', { noun: `${t('noun.media_post')} #${id}` }))
  ...
  const handleCreate = useCallback(
    async (data: Record<string, unknown>) => {
      ...
      toast.success(t('toast.created', { noun: `${t('noun.media_post')} #${res.data.id}` }))
      setRefreshTick((t) => t + 1)
    },
    [toast],
  )
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      ...
      toast.success(t('toast.deleted', { noun: `${t('noun.media_post')} #${id}` }))
      setDeleting(null)
      setRefreshTick((t) => t + 1)
    },
    [toast],
  ...
  return (
    <div className="stack">
      ...
      <ConfirmDialog
        open={deleting !== null}
        title={deleting ? t('common.confirm_delete_title', { noun: t('noun.media_post'), id: deleting.id }) : ''}
