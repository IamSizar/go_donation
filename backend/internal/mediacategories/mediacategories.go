// Package mediacategories backs the admin-managed "Our Work" media-category
// list (#22): an ordered, 4-language taxonomy for the News & Activities feed.
// Each media post can be tagged with one category; the app shows filter chips
// and a per-post category label. Mirrors the projectcategories CMS pattern.
...
// Category is one media category with its 4-language display names.
...
func (s *Store) Add(ctx context.Context, c Category, actorID *int64) (*Category, error) {
	...
	err := s.Pool.QueryRow(ctx,
		`INSERT INTO media_categories (slug, name_en, name_ar, name_ckb, name_kmr, active, created_by)
...
func (s *Store) Reorder(ctx context.Context, orderedIDs []int64) error {
	...
	for i, id := range orderedIDs {
		if _, err := tx.Exec(ctx,
			`UPDATE media_categories SET display_order = $2 WHERE id = $1`, id, i+1); err != nil {
			return err
		}
	}
...
// Delete removes a category. Existing posts keep their category_slug text; the
// category simply drops out of the filter chips.
func (s *Store) Delete(ctx context.Context, id int64) error {
	ct, err := s.Pool.Exec(ctx, `DELETE FROM media_categories WHERE id = $1`, id)
	if err != nil {
		return err
	}
