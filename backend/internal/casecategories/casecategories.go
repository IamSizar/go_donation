// Package casecategories backs the admin-managed beneficiary-case category
// list (Quick Filter Capsules): an ordered, 4-language taxonomy for
// beneficiary cases (Orphan Sponsorship, Widow Support, Food Baskets, etc.).
// Each case can be tagged with one category; the app shows capsule filter
// chips on Home that jump into the case list pre-filtered. Mirrors the
// mediacategories CMS pattern.
package casecategories
...
// Category is one case category with its 4-language display names.
...
// activeOnly, only active rows (used by the app's capsule filter row).
...
func (s *Store) Add(ctx context.Context, c Category, actorID *int64) (*Category, error) {
	...
	err := s.Pool.QueryRow(ctx,
		`INSERT INTO case_categories (slug, name_en, name_ar, name_ckb, name_kmr, active, created_by)
...
func (s *Store) Reorder(ctx context.Context, orderedIDs []int64) error {
	...
	for i, id := range orderedIDs {
		if _, err := tx.Exec(ctx,
			`UPDATE case_categories SET display_order = $2 WHERE id = $1`, id, i+1); err != nil {
			return err
		}
	}
...
// Delete removes a category. Existing cases keep their category_slug text;
// the category simply drops out of the capsule filter row.
func (s *Store) Delete(ctx context.Context, id int64) error {
	ct, err := s.Pool.Exec(ctx, `DELETE FROM case_categories WHERE id = $1`, id)
	if err != nil {
		return err
	}
